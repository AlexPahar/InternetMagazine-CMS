...
$result = mysql_query("SELECT * FROM cart,table_products WHERE cart.cart_ip = '{$_SERVER['REMOTE_ADDR']}' AND table_products.product_id = cart.cart_id_products",$link);
If (mysql_num_rows($result) > 0)
{
$row = mysql_fetch_array($result);
...
';
 ...
 echo '
 <h2 class="itog-price" align="right">Сумма: <strong>'.group_numerals($all_price).'</strong> руб</h2>
...
{
    echo '<h3 id="clear-cart" align="center">Очистить корзину</h3>';
}
...
<li><label class="order_label_style" for="order_address"><span>*</span>Адрес<br /> Доставки</label><input type="text" name="order_address" id="order_address" value="'.$_SESSION["order_address"].'" /><span>Пример: с. Волчиха,<br /> ул 1-Мая 216</span></li>
';
}
echo '
<li><label class="order_label_style" for="order_note">Примечание</label><textarea name="order_note"  >'.$_SESSION["order_note"].'</textarea><span>Уточняйте информацию о заказе.</span></li>
</ul>
...
echo '
<ul id="list-info" >
<li><strong>Проверка данных:</strong>'.$_SESSION['order_delivery'].'</li>
...
';    
 ...
 echo '
<h2 class="itog-price" align="right">Сумма: <strong>'.$itogpricecart.'</strong> руб</h2>
  <p align="right" class="button-next" ><a href="" >Далее</a></p> 
